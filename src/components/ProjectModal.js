...
const ProjectModal = ({ isOpen, onClose, name, image, techStacks, longDescription, githubLink, demoLink }) => {
    ...
    return (
        <div className={`fixed top-0 left-0 w-full h-full bg-gray-600 bg-opacity-50 ${isDarkModeOpen ? 'text-white' : ''} ${isOpen ? 'block' : 'hidden'}`}>
            <div ref={modalRef} className={`modal-container mx-auto p-4 max-w-lg mt-16 flex flex-col items-center justify-center z-30 border rounded-md ${isDarkModeOpen ? 'border-white bg-gray-800 ' : 'border-black bg-white'}`} style={{ maxHeight: '100vh' }}>
