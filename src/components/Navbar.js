...
const Navbar = () => {
 ...
 import('../assets/CV.pdf').then((module) => {
  ...
  return (
    <nav className={`${isDarkModeOpen ? 'bg-gray-900 shadow-sm shadow-gray-100' : 'bg-white shadow-lg'}  relative `}>
...
const SidebarList = ({ navRef ,scrollToSection}) => {
  ...
  return (
    <ul
      ref={navRef}
      className={`absolute top-full right-3 border-2 p-2 rounded-lg shadow-lg z-10 sm:hidden md:hidden lg:hidden ${isDarkModeOpen ? 'border-gray-800 bg-black' : 'border-gray-400 bg-white'} mt-2 `}
    >
